...
class DataTrainingArguments:
    ...
    test_split_name: Optional[str] = field(
        default="test",
        metadata={
            "help": "The name of the testing data set split to use (via the datasets library). Defaults to 'test'"
        },
    )
...
def create_vocabulary_from_data(datasets: DatasetDict):
    ...
    vocabs.pop('test', None)  # warning: here assume test split's name is test!!!
...
def main():
    ...
    raw_datasets["test"] = load_dataset(
        data_args.dataset_name, data_args.dataset_config_name, split=data_args.test_split_name,
        cache_dir="./cache"
    )
    ...
    if training_args.do_eval:
        ...
        logger.info("*** Testing ***")
        pred = trainer.predict(test_dataset=vectorized_datasets["test"])
        metrics = pred.metrics
        metrics["test_samples"] = min(max_eval_samples, len(vectorized_datasets["test"]))
        ...
        for pred_s, label_s in zip(pred_str, label_str):
            lines.append(f"Prediction: {pred_s}\tGroundtruth: {label_s}")

        with open(f"{training_args.output_dir}/test_results.txt", 'w') as f:
            f.write('\n'.join(lines))
